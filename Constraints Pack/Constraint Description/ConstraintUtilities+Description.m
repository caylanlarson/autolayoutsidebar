...
CGPoint GetLayoutPoint(NSLayoutConstraint *constraint, UIView *view, UIView *callingView, NSLayoutAttribute attribute)
{
    ...
    if (![view isEqual:constraint.SE_firstView])
        otherView = constraint.SE_firstView;
    if (constraint.SE_secondView && ![view isEqual:constraint.SE_secondView])
        otherView = constraint.SE_secondView;
...
{
    return GetLayoutPoint(constraint, constraint.SE_firstView, self, constraint.firstAttribute);
}

- (CGPoint) secondPoint: (NSLayoutConstraint *) constraint
{
    UIView *v2 = constraint.SE_secondView ? : constraint.SE_firstView;
...
- (void) drawConstraintLinesOnPrimaryView
{
    ...
    for (NSLayoutConstraint *c in self.SE_allConstraints)
    {
